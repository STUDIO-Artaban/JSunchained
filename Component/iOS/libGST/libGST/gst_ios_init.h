#ifndef __GST_IOS_INIT_H__
#define __GST_IOS_INIT_H__

#include <gst/gst.h>

G_BEGIN_DECLS

#define GST_G_IO_MODULE_DECLARE(name) \
extern void G_PASTE(g_io_module_, G_PASTE(name, _load_static)) (void)

#define GST_G_IO_MODULE_LOAD(name) \
G_PASTE(g_io_module_, G_PASTE(name, _load_static)) ()

/* Uncomment each line to enable the plugin categories that your application needs.
 * You can also enable individual plugins. See gst_ios_init.m to see their names
 */

#define GST_IOS_PLUGIN_COREELEMENTS
//#define GST_IOS_PLUGIN_TYPEFINDFUNCTIONS
//#define GST_IOS_PLUGIN_AUDIOCONVERT
//#define GST_IOS_PLUGIN_AUDIORESAMPLE
#define GST_IOS_PLUGIN_VIDEOCONVERT

#define GST_IOS_PLUGIN_APP
//////#define GST_IOS_PLUGINS_CORE

//#define GST_IOS_PLUGIN_PLAYBACK

//#define GST_IOS_PLUGIN_FAAD
//#define GST_IOS_PLUGIN_VOAACENC
//#define GST_IOS_PLUGIN_X264
//#define GST_IOS_PLUGIN_LIBAV
//////#define GST_IOS_PLUGINS_CODECS_RESTRICTED

//#define GST_IOS_PLUGIN_MULTIFILE
//////#define GST_IOS_PLUGINS_EFFECTS

//#define GST_IOS_PLUGIN_VORBIS
//#define GST_IOS_PLUGIN_OGG
//#define GST_IOS_PLUGIN_ISOMP4
#define GST_IOS_PLUGIN_JPEG
//#define GST_IOS_PLUGIN_WAVENC
//#define GST_IOS_PLUGIN_WAVPARSE
//#define GST_IOS_PLUGIN_AUDIOPARSERS
//////#define GST_IOS_PLUGINS_CODECS

void gst_ios_init();

G_END_DECLS

#endif
